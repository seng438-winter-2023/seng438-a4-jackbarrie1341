...
- Checkpoints are used for verification during testing. They are used to verify the behaviour of the system which is used to determine the state of the system and furthermore, if the test will pass or fail. Assertions are an example of checkpoints.
- A checkpoint is a verification operation that is performed during testing. It is used to analyze behaviours and state to help determine hese operations are an essential part of the testing process as they check whether the tested application functions properly.
# how did you test each functionaity with different test data
-For each functionality selected, we carried out tests using boundary value data, invalid, valid as well as with the website in different states to ensure homogenity in the outcomes of the. For login, we used correct login info, wrong login info; for signout, we signed in before signin out and signed out after a new user had signed up. Search, addtocart, redirect.
